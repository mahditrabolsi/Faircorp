...
class WindowControllerTest {
    ...
    void shouldLoadAWindowAndReturnNullIfNotFound() throws Exception {
        given(windowDao.findById(-9L)).willReturn(Optional.empty());
    ...
    void shouldLoadAWindow() throws Exception {
        given(windowDao.findById(-9L)).willReturn(Optional.of(createWindow("window 1")));
    ...
    void shouldSwitchWindow() throws Exception {
        ...
        given(windowDao.findById(-9L)).willReturn(Optional.of(expectedWindow));
    ...
    void shouldDeleteWindow() throws Exception {
        mockMvc.perform(delete("/api/windows/-9"))
                .andExpect(status().isOk());
    }

    private Window createWindow(String name) {
        Room room = new Room(2, "Room 1");
